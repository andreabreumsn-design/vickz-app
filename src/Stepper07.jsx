...
export default function Stepper07() {
  const steps = [
    ...
    { num: 5, label: 'Fotos', status: 'active' },
    { num: 6, label: 'Declaração', status: 'pending' },
  ];
        ...
        <div className="flex items-center justify-between gap-0.5">
          {steps.map((step, idx) => (
            <div key={step.num} className="flex items-center flex-1 min-w-0">
              <div className={`
                w-7 h-7 rounded-full flex items-center justify-center font-bold text-xs transition-all flex-shrink-0
